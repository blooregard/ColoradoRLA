...
public final class ComparisonAuditController {
   ...
   * for the given county dashboard in the specified range in the audit
   * sequence.
   * 
   ...
   * @return the list of sequence numbers, of size the_max_index - the_min_index
   *         + 1; the first element of this list will be the "min_index"th
   *         ballot card to audit, and the last will be the "max_index"th.
   */
  public static List<Integer> getCVRSeqNumsInAuditSequence(final County the_county,
    ...
    final PseudoRandomNumberGenerator prng =
        new PseudoRandomNumberGenerator(seed, with_replacement, minimum, maximum);
   ...
   */
  public static List<CastVoteRecord> getCVRsForSequenceNumbers(final County the_county,
  ...
  /**
   * Get the cast vote records to audit, in order, for the given county in the
   * specified range in the audit sequence.
   * 
   * @param the_county The county.
   * @param the_min_index The minimum index to return.
   * @param the_max_index The maximum index to return.
   * @return the list of ballot cards, of size the_max_index - the_min_index +
   *         1; the first element of this list will be the "min_index"th ballot
   *         card to audit, and the last will be the "max_index"th.
   */
  ...
  /**
   * Get the cast vote records to audit, in order, for the given county in the
   * specified political party in the audit sequence.
   * 
   * @param the_county The county.
   * @param the_min_index The minimum index to return.
   * @param the_max_index The maximum index to return.
  ...
  public static List<CastVoteRecord> getCVRsInAuditSequence(final County the_county,
                                                            ...
                                                            final int the_max_index) {
    final List<PartyBallotType> partyBallots =
        PartyBallotTypeQueries.matching(the_county, the_party);
    final List<String> ballotStyles =
        partyBallots.stream().map(pb -> pb.ballotType()).collect(Collectors.toList());
    final List<CastVoteRecord> cvrs = CastVoteRecordQueries.get(the_county.id(), ballotStyles);
    final String seed =
        Persistence.getByID(DoSDashboard.ID, DoSDashboard.class).auditInfo().seed();

    final boolean with_replacement = false;
    final int minimum = 0;
    final int maximum = cvrs.size() - 1;

    final PseudoRandomNumberGenerator prng =
        new PseudoRandomNumberGenerator(seed, with_replacement, minimum, maximum);
    final List<Integer> indices = prng.getRandomNumbers(0, the_max_index);

    final List<CastVoteRecord> result = new ArrayList<>();

    for (final int index : indices) {
      result.add(cvrs.get(index));
    }
   ...
   *         list if there are no CVRs remaining to audit or if no round is in
   *         progress.
   */
  public static List<Long> cvrIDsRemainingInCurrentRound(final CountyDashboard the_cdb) {
    ...
    if (round != null) {
      for (int i = 0; i + round.actualAuditedPrefixLength() < round
          .expectedAuditedPrefixLength(); i++) {
   ...
   * @param the_audited true to include already-audited ballots, false
   *          otherwise.
   */
  ...
  public static List<CastVoteRecord> computeBallotOrder(final CountyDashboard the_cdb,
    ...
    while ((the_duplicates && cvr_to_audit_list.size() < the_ballot_count) ||
      ...
      for (int i = 0; i < new_cvrs.size(); i++) {
        ...
        if ((the_duplicates || !cvr_set.contains(cvr)) && (the_audited || !cvr.auditFlag())) {
          cvr_to_audit_list.add(cvr);
        }
        cvr_set.add(cvr);
      }
      start = end + 1; // end is inclusive
      end = start + (the_ballot_count - cvr_to_audit_list.size()) - 1; 
    }
  ...
  /**
   * Compute the ballot (cards) for audit, for a particular county dashboard, a
   * particular start index, and a particular desired audit prefix length.
   * 
   * @param the_cdb The dashboard.
   * @param the_start_index The start index.
   ...
   *         in any ballot (cards), as when the prefix length or start index is
   *         invalid, the returned list is empty.
   */
  @SuppressWarnings("PMD.UselessParentheses")
  public static List<CastVoteRecord> computeBallotOrder(final CountyDashboard the_cdb,
                                                        final int the_start_index,
    ...
    final List<CastVoteRecord> cvrs = getCVRsInAuditSequence(the_cdb.county(), the_start_index,
                                                             the_desired_prefix_length - 1); 
    final Set<CastVoteRecord> cvr_set = new HashSet<>();
    ...
    if (the_start_index > 0) {
      previous_cvr_set
          .addAll(getCVRsInAuditSequence(the_cdb.county(), 0, the_start_index - 1));
    }
  ...
  /**
   * Compute the ballot (cards) for audit, for a particular county and round.
   * The returned list does not have duplicates, and is in _arbitrary order_.
   * 
   * @param the_cdb The dashboard.
   ...
   *          otherwise.
   * @return the list of ballot (cards) for audit; if the query does not result
   *         in any ballot (cards), as when the round number is invalid, the
  ...
  /**
   * Initializes the audit data for the specified county dashboard and starts
   * its first audit round.
   * 
   * @param the_cdb The dashboard.
   * @return true if an audit round is started for the dashboard, false
   *         otherwise; an audit round might not be started if there are no
   *         driving contests in the county, or if the county needs to audit 0
   *         ballots to meet the risk limit.
   */
  public static boolean initializeAuditData(final CountyDashboard the_cdb) {
    ...
    final Map<Contest, Integer> county_driving_contests = new HashMap<>();
    int to_audit;

    initializeAuditContests(the_cdb, dosdb, risk_limit, election, all_driving_contests,
                            county_driving_contests);

    Main.LOGGER.info("driving contests setting: " + county_driving_contests);
    the_cdb.setDrivingContests(county_driving_contests.keySet());

    if (election == ElectionType.primary) {
      to_audit = county_driving_contests.values().stream().mapToInt(Number::intValue).sum();
      ...
      if (!county_driving_contests.isEmpty() && 0 < to_audit) {
        ...
        final List<CastVoteRecord> cvrs_to_audit = new ArrayList<>();

        for (final Map.Entry<Contest, Integer> e : county_driving_contests.entrySet()) {
          final PoliticalParty p = PoliticalParty.valueOf(e.getKey().description());
          cvrs_to_audit.addAll(getCVRsInAuditSequence(the_cdb.county(), p, e.getValue() - 1));
        }

        initializeAuditRound(the_cdb, to_audit, cvrs_to_audit);
      } else {
        result = false;
      }
    } else {
      to_audit = county_driving_contests.values().stream().mapToInt(Number::intValue).max()
          .getAsInt();
      the_cdb.setEstimatedSamplesToAudit(Math.max(0, to_audit));
      the_cdb.setOptimisticSamplesToAudit(Math.max(0, to_audit));
      if (!county_driving_contests.isEmpty() && 0 < to_audit) {
   ...
   * @param
   */
  private static void initializeAuditContests(final CountyDashboard the_cdb,
                                              final DoSDashboard dosdb,
                                              final BigDecimal risk_limit,
                                              final ElectionType election,
                                              final Set<Contest> all_driving_contests,
                                   final Map<Contest, Integer> county_driving_contests) {
    ...
    for (final CountyContestResult ccr : CountyContestResultQueries
        .forCounty(the_cdb.county())) {
      ...
      final CountyContestComparisonAudit audit =
          new CountyContestComparisonAudit(the_cdb, ccr, risk_limit, reason, election);
      ...
      if (election == ElectionType.primary && PoliticalParty.contains(contest.description())) {
        ...
        final List<PartyBallotType> partyBallots =
            PartyBallotTypeQueries.matching(the_cdb.county(), party);
        final List<String> ballotStyles =
            partyBallots.stream().map(pb -> pb.ballotType()).collect(Collectors.toList());
        final List<CastVoteRecord> cvrs =
            CastVoteRecordQueries.get(the_cdb.county().id(), ballotStyles);

        ccr.setPartyBallotCount(cvrs.size());
        Persistence.saveOrUpdate(ccr);
      }

      comparison_audits.add(audit);
      if (all_driving_contests.contains(contest)) {
        county_driving_contests.put(contest, audit.initialSamplesToAudit());
      }
    }
  ...
  /**
   * Takes the auditable CVR lines and removes duplicates and primes the audit
   * round
   * 
   * @param the_cdb The dashboard.
   * @param to_audit The number of ballots to audit in this round
   * @param cvrs_to_audit The CVR lines to be included in this round
   */
  private static void initializeAuditRound(final CountyDashboard the_cdb, final int to_audit,
                                           final List<CastVoteRecord> cvrs_to_audit) {
    ...
    the_cdb.startRound(sorted_deduplicated_cvrs.size(), to_audit, 0, ballot_ids_to_audit,
  ...
  /**
   * Starts a new round on the specified dashboard with the specified number of
   * physical ballots.
   * 
   * @param the_cdb The dashboard.
   * @param the_round_length The count.
   * @param the_multiplier The multiplier.
   * @return true if a round is started, false otherwise (a round might not be
   *         started because the risk limit has already been achieved).
  ...
  public static boolean startNewRoundOfLength(final CountyDashboard the_cdb,
    ...
    while (!extra_cvrs.isEmpty() && sorted_deduplicated_new_cvrs.size() < the_round_length) {
      extra_cvrs =
    ...
    if (sorted_deduplicated_new_cvrs.isEmpty()) {
      ...
      Main.LOGGER.info("starting audit round " + (rounds.size() + 1) + " for county " +
                       the_cdb.id() + " at audit sequence number " + start_index + " with " +
                       sorted_deduplicated_new_cvrs.size() + " ballots to audit");
      final List<Long> ballot_ids_to_audit = new ArrayList<>();
      for (final CastVoteRecord cvr : sorted_deduplicated_new_cvrs) {
        ballot_ids_to_audit.add(cvr.id());
      }
      the_cdb.startRound(sorted_deduplicated_new_cvrs.size(), start_index + new_cvrs.size(),
   ...
   *         started because the risk limit has already been achieved).
   * @exception IllegalStateException if a round cannot be started from
   *              estimates because there are no previous rounds or because
   *              there are no CVRs.
   */
  public static boolean startNewRoundFromEstimates(final CountyDashboard the_cdb,
    ...
    if (the_cdb.ballotsAudited() == cvr_count.getAsLong()) {
      ...
      Main.LOGGER.info("starting audit round " + (rounds.size() + 1) + " for county " +
                       the_cdb.id() + " at audit sequence number " + start_index + " with " +
                       round_length + " ballots to audit");
      the_cdb.startRound(round_length, expected_prefix_length, start_index,
                         ballot_ids_to_audit, new_cvr_ids);
      updateRound(the_cdb, the_cdb.currentRound());
      updateCVRUnderAudit(the_cdb);
      result = true;
    }
  ...
  /**
   * Submit an audit CVR for a CVR under audit to the specified county
   * dashboard.
   * 
   * @param the_cdb The dashboard.
   * @param the_cvr_under_audit The CVR under audit.
   * @param the_audit_cvr The corresponding audit CVR.
   * @return true if the audit CVR is submitted successfully, false if it
   *         doesn't correspond to the CVR under audit, or the specified CVR
   *         under audit was not in fact under audit.
   */
  ...
  public static boolean submitAuditCVR(final CountyDashboard the_cdb,
    ...
    if (info == null) {
      Main.LOGGER.info("attempt to submit ACVR for county " + the_cdb.id() + ", cvr " +
      ...
      if (info.acvr() == null) {
        ...
        the_cdb.setAuditedSampleCount(the_cdb.auditedSampleCount() + new_count);
      } else {
        // the record has been audited before, so we need to "unaudit" it
        final int former_count = unaudit(the_cdb, info);
        info.setACVR(the_audit_cvr);
        final int new_count = audit(the_cdb, info, true);
        the_cdb.setAuditedSampleCount(the_cdb.auditedSampleCount() - former_count + new_count);
      }
      result = true;
    } else {
      Main.LOGGER.info("attempt to submit non-corresponding ACVR " + the_audit_cvr.id() +
                       " for county " + the_cdb.id() + ", cvr " + the_cvr_under_audit.id());
    }
    Persistence.flush();
    updateCVRUnderAudit(the_cdb);
    the_cdb.setEstimatedSamplesToAudit(computeEstimatedSamplesToAudit(the_cdb) -
                                       the_cdb.auditedSampleCount());
    the_cdb.setOptimisticSamplesToAudit(computeOptimisticSamplesToAudit(the_cdb) -
                                        the_cdb.auditedSampleCount());
   ...
   * increased by the percentage of discrepancies seen in the audited ballots so
   * far.
   * 
   * @param the_cdb The dashboard.
   */
  public static int computeEstimatedSamplesToAudit(final CountyDashboard the_cdb) {
   ...
   * @param the_cdb The dashboard.
   */
  public static int computeOptimisticSamplesToAudit(final CountyDashboard the_cdb) {
    int to_audit = Integer.MIN_VALUE;
    final Set<Contest> driving_contests = the_cdb.drivingContests();
    for (final CountyContestComparisonAudit ccca : the_cdb.comparisonAudits()) {
   ...
   * dashboard. This check sets the audit flag on the CVR record in memory, so
   * its result can be accessed later without an expensive database hit.
   * 
   * @param the_cdb The county dashboard.
   * @param the_cvr The CVR.
   * @return true if the specified CVR has been audited, false otherwise.
   */
  public static boolean audited(final CountyDashboard the_cdb, final CastVoteRecord the_cvr) {
  ...
  /**
   * Updates a round object with the disagreements and discrepancies that
   * already exist for CVRs in its audit subsequence, creates any CVRAuditInfo
   * objects that don't exist but need to, and increases the multiplicity of any
   * CVRAuditInfo objects that already exist and are duplicated in this round.
   */
  private static void updateRound(final CountyDashboard the_cdb, final Round the_round) {
    for (final Long cvr_id : new HashSet<>(the_round.auditSubsequence())) {
      ...
      if (cvrai == null) {
        ...
        final int multiplicity = Collections.frequency(the_round.auditSubsequence(), cvr_id);
  ...
  /**
   * Audits a CVR/ACVR pair by adding it to all the audits in progress. This
   * also updates the local audit counters, as appropriate.
   */
  @SuppressWarnings({"PMD.ModifiedCyclomaticComplexity", "PMD.StdCyclomaticComplexity",
      "PMD.NPathComplexity"})
  private static int audit(final CountyDashboard the_cdb, final CVRAuditInfo the_info,
  ...
  /**
   * "Unaudits" a CVR/ACVR pair by removing it from all the audits in progress
   * in the specified county dashboard. This also updates the dashboard's
  ...
  /**
   * Updates the current CVR to audit index of the specified county dashboard to
   * the first CVR after the current CVR under audit that lacks an ACVR. This
   * "audits" all the CVR/ACVR pairs it finds in between, and extends the sequence 
   * of ballots to audit if it reaches the end and the audit is not concluded.
   */
  ...
  /**
   * Checks that the specified CVR and ACVR are an audit pair, and that the
   * specified ACVR is auditor generated.
   * 
   * @param the_cvr The CVR.
  ...
  private static boolean checkACVRSanity(final CastVoteRecord the_cvr,
    ...
    return the_cvr.isAuditPairWith(the_acvr) && the_acvr.recordType().isAuditorGenerated();
  }
}
